...
const inputCls =
  "w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-text placeholder-text-dim focus:outline-none focus:border-bloom/60 focus:bg-white/[0.05] transition-all";
const labelCls =
  "block text-xs font-medium text-text-muted mb-1.5 uppercase tracking-wide";
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className={labelCls}>
          Recipe <span className="text-text-dim normal-case">(optional)</span>
        </label>
      ...
      {beans.length > 0 && (
        <div>
          <label className={labelCls}>
            Beans <span className="text-text-dim normal-case">(optional)</span>
          </label>
          <BeanTagSelector
            beans={beans}
            selected={selectedBeans}
            onChange={setSelectedBeans}
          />
