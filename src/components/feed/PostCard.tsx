      ...
      <div className="px-4 pt-3 pb-4 space-y-2.5">
        ...
        {post.caption && (
          <p className="text-[13px] text-text leading-relaxed">
            <Link
              href={`/profile/${username}`}
              className="font-semibold hover:text-bloom transition-colors"
            >
              {displayName}
            </Link>{" "}
              ...
              (bean) =>
                bean && (
                  <Link
                    key={bean.id}
                    href={`/beans/${bean.id}`}
                    className="inline-block px-2 py-0.5 rounded-full text-[11px] font-medium bg-surface-raised text-text-muted border border-border hover:text-bloom hover:border-bloom-dim/30 tap-scale tag-glow transition-colors"
                  >
                    {bean.name}
